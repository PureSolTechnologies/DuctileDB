...
public class OperationsHelper {
	...
	typeIndexPut.addColumn(DatabaseColumnFamily.INDEX.getNameBytes(), IdEncoder.encodeRowId(vertexId), new byte[0]);
	return typeIndexPut;
    }
	...
	typeIndexDelete.addColumns(DatabaseColumnFamily.INDEX.getNameBytes(), IdEncoder.encodeRowId(vertexId));
	...
	indexPut.addColumn(DatabaseColumnFamily.INDEX.getNameBytes(), IdEncoder.encodeRowId(vertexId),
	...
	indexDelete.addColumns(DatabaseColumnFamily.INDEX.getNameBytes(), IdEncoder.encodeRowId(vertexId));
	return indexDelete;
    }

    public static Put createEdgeTypeIndexPut(long edgeId, String type) {
	Put typeIndexPut = new Put(Bytes.toBytes(type));
	typeIndexPut.addColumn(DatabaseColumnFamily.INDEX.getNameBytes(), IdEncoder.encodeRowId(edgeId), new byte[0]);
	return typeIndexPut;
    }

    public static Put createEdgePropertyIndexPut(long edgeId, String key, Serializable value) throws IOException {
	Put indexPut = new Put(Bytes.toBytes(key));
	indexPut.addColumn(DatabaseColumnFamily.INDEX.getNameBytes(), IdEncoder.encodeRowId(edgeId),
		Serializer.serializePropertyValue(value));
	return indexPut;
    }
	...
	Delete typeIndexDelete = new Delete(Bytes.toBytes(type));
	typeIndexDelete.addColumns(DatabaseColumnFamily.INDEX.getNameBytes(), IdEncoder.encodeRowId(edgeId));
	return typeIndexDelete;
    }

    public static Delete createEdgePropertyIndexDelete(long edgeId, String key) {
	Delete indexDelete = new Delete(Bytes.toBytes(key));
	indexDelete.addColumns(DatabaseColumnFamily.INDEX.getNameBytes(), IdEncoder.encodeRowId(edgeId));
	return indexDelete;
    }
